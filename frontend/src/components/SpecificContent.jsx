  ...
  return (
    <div className="w-[100%] flex flex-col gap-7 mt-10 justify-center items-center overflow-hidden">
      <div className="w-[92%] flex justify-center items-center ">
        <h1 className="text-white text-4xl font-bold ">React.js</h1>
      </div>
      <div className="flex gap-4  w-[100%]   justify-center items-center   ">
        <AiFillLeftCircle color="white" size={30} onClick={handleScrollLeft} />
        <div
          ref={scrollBar}
          className="w-[100%] p-3    flex    gap-4  overflow-scroll scroll-smooth  scrollbar-hide  "
        >
          {listOfSpecificContent.map((item, index) => {
            return <SpecificCard object={item} key={index} />;
          })}
        </div>
        <AiFillRightCircle
          color="white"
